...
function buildCachedDeviceRecord(
  ...
  return {
    ...
    latestSequence: remote.latestSequence ?? existing?.latestSequence ?? 0,
    recordedAt: remote.recordedAt ?? existing?.recordedAt ?? remote.lastSeenAt ?? Date.now(),
    rtcIso: remote.rtcIso ?? existing?.rtcIso ?? null,
    sdCardMounted: remote.sdCardMounted ?? existing?.sdCardMounted ?? false,
    timeSource: remote.timeSource ?? existing?.timeSource ?? "unknown",
